...
int main()
{
    std::string filename = "resultados_sequential.csv";
    std::ofstream file(filename, std::ios::app);

    std::vector<int> dimensions = {1000, 2000, 3000, 4000, 8000};
    int iterations = 3;
    file << "dimension,tiempo" << std::endl;
    ...
    for (auto& dimension: dimensions){
      for (int i = 0; i < iterations; i++)
      {
        ...
        A.rand();
        B.rand();
        std::cout << "Ejecutando experimento para dimension: " << dimension << std::endl;
        start = std::chrono::high_resolution_clock::now();
        A* B;
        end = std::chrono::high_resolution_clock::now();
        total_time += end - start;
      }
      std::cout << "Tiempo promedio: " << total_time.count() / iterations << std::endl;
      file << dimension << "," << total_time.count() / iterations << std::endl;
